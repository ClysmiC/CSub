...
public class FFWButton extends JPanel implements MouseListener {	
	private CSub csub;
	private int width;
	private int height;
	...
	public FFWButton(CSub c) {
	    csub = c;
	
	    width = InterfacePanel.FFW_BUTTON_WIDTH;
        ...
        try {
        	...
        	InputStream input = classLoader.getResourceAsStream("ffw.png");
        	image = ImageIO.read(input);
        } catch (IOException e) {
        	System.out.println("Failed to load image");
        }
	    
	    setToolTipText("Rewind (hold)");
