...
public class CenterButton extends JPanel implements MouseListener {
	private CSub csub;
    private int width;
    private int height;
    ...
    public CenterButton(CSub c) {
        csub = c;

        width = WindowButtonPanel.CENTER_BUTTON_WIDTH;
        height = CSub.WINDOW_BUTTON_PANEL_HEIGHT;
        setPreferredSize(new Dimension(width, height));
        
        backgroundColor = CSub.DEFAULT_COLOR;
        
        //get the image for the center button
        try {
        	ClassLoader classLoader = Thread.currentThread().getContextClassLoader();
        	InputStream input = classLoader.getResourceAsStream("center.png");
    ...
    public void mouseClicked(MouseEvent e) {
    	Dimension screenSize = csub.getScreenSize();
    	int newX = (int)screenSize.getWidth();
    	newX = newX / 2 - csub.getWidth() / 2;
